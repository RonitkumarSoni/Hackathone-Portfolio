...
const questions = {
  ...
  Fun: 'Can you show me your hackathon projects and experiences?',
  Contact:
    'How can I contact you? What are your email and social links?',
  Certificates: 'What certifications do you have? Show me your certificates.',
};
...
const questionConfig = [
  ...
  { key: 'Certificates', color: '#E5A50A', icon: Award },
      ...
      <Drawer.Root open={open} onOpenChange={setOpen}>
        <div className="w-full">
          ...
          {isVisible && (
            <div className="w-full">
              <div
                className="flex w-full flex-wrap gap-1 md:gap-3 justify-center"
              >
                {questionConfig.map(({ key, color, icon: Icon }) => (
                  <Button
                    key={key}
                    onClick={() => !(hasReachedLimit || isLoading) && handleQuestionClick(key)}
                    variant="outline"
                    className={`h-auto min-w-[80px] md:min-w-[95px] flex-shrink-0 rounded-xl border px-2 md:px-3 py-3 shadow-none backdrop-blur-sm transition-none ${(hasReachedLimit || isLoading)
                ...
                <TooltipProvider>
                  <Tooltip delayDuration={0}>
                    <TooltipTrigger asChild>
                      <Drawer.Trigger className="group relative flex flex-shrink-0 items-center justify-center" disabled={hasReachedLimit || isLoading}>
                        <motion.div
                          className={`flex h-auto min-w-[40px] md:min-w-[48px] items-center justify-center rounded-xl border px-2 md:px-3 py-3 text-sm backdrop-blur-sm transition-all duration-200 ${(hasReachedLimit || isLoading)
                              ? 'cursor-not-allowed border-gray-200 bg-gray-100 opacity-50'
                              : 'hover:bg-border/30 cursor-pointer border-neutral-200 bg-white/80 dark:border-neutral-800 dark:bg-neutral-900'
                            }`}
                          whileHover={!(hasReachedLimit || isLoading) ? { scale: 1 } : {}}
                          whileTap={!(hasReachedLimit || isLoading) ? { scale: 0.98 } : {}}
                        >
                          <div className="flex items-center gap-3 text-gray-700">
                            <CircleEllipsis
